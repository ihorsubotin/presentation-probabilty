...
function set_up_timeupdate(fragment) {
  ...
  if (prev_frag && prev_frag.getAttribute("fragment_type") == "complete_loop") {
    ...
    if (video_elem.currentTime < parseFloat(end_prev)) {
  ...
  video_elem.currentTime = time_start.toString();
  //video_elem.setAttribute('preload', 'auto');
  video_elem.play();
  video_elem.ontimeupdate = function() {
    if (video_elem.currentTime - parseFloat(time_end) > 0){
      video_elem.pause();
      video_elem.currentTime = time_end.toString();
